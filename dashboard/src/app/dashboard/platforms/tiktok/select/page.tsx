...
function TikTokSelectAccountContent() {
  ...
  const [accounts, setAccounts] = useState<TikTokAdAccount[]>([]);
  ...
  async function fetchPendingTokenData() {
    try {
      const response = await fetch(
        `/api/auth/tiktok/pending?id=${pendingTokenId}`,
      );
  ...
  async function handleSubmit() {
    ...
    try {
      const response = await fetch("/api/auth/tiktok/select-account", {
        method: "POST",
      ...
      toast({
        ...
        description: "Your TikTok Ads account has been successfully connected.",
      });

      router.push("/dashboard/platforms?success=true&platform=tiktok_ads");
    } catch (err) {
      toast({
  ...
  return (
    <div className="container max-w-2xl py-8">
      <Card>
        <CardHeader>
          <CardTitle>Select TikTok Ad Account</CardTitle>
          <p className="text-sm text-muted-foreground">
            Choose which TikTok advertiser account you want to connect to the
            dashboard.
          </p>
        </CardHeader>
...
export default function TikTokSelectAccountPage() {
  return (
    <Suspense
      fallback={
        <div className="flex items-center justify-center min-h-[400px]">
