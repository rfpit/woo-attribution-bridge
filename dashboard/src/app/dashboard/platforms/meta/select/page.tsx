...
function MetaSelectAccountContent() {
  ...
  async function fetchPendingTokenData() {
    try {
      const response = await fetch(
        `/api/auth/meta/pending?id=${pendingTokenId}`,
      );
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      router.push("/dashboard/platforms?success=true&platform=meta_ads");
    } catch (err) {
      toast({
  ...
  return (
    <div className="container max-w-2xl py-8">
      <Card>
        <CardHeader>
          <CardTitle>Select Meta Ad Account</CardTitle>
          <p className="text-sm text-muted-foreground">
            Choose which Meta ad account you want to connect to the dashboard.
          </p>
        </CardHeader>
...
export default function MetaSelectAccountPage() {
  return (
    <Suspense
      fallback={
        <div className="flex items-center justify-center min-h-[400px]">
