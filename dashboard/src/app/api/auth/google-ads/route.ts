 ...
 * DELETE: Disconnect Google Ads account
 */
...
import { buildAuthUrl, revokeToken } from "@/lib/google-ads";
 ...
 *
 * Initiates the Google Ads OAuth flow by:
 * 1. Generating a secure state token
 * 2. Storing it in an encrypted cookie
 * 3. Redirecting to Google's consent screen
 */
export async function GET(): Promise<NextResponse> {
  ...
  // Build and redirect to Google OAuth URL
...
/**
 * DELETE /api/auth/google-ads?connectionId=xxx
 *
 * Disconnects a Google Ads account by:
 * 1. Revoking the token with Google
